...
{ 
  ...
  if(starttime==0 && force_lcd_update) {
      lcd.setCursor(0,1);
      lcd.print("00h 00m");
  }
  
  if(card.filename && force_lcd_update) {
      lcd.setCursor(0,2);
      lcdprintPGM("");
  ...
  */
  ...
  if(messagetext[0]!='\0')
  {
    ...
    for(int8_t i=0;i<LCD_WIDTH-n;i++)
      lcd.print(" ");
    messagetext[0]='\0';
  } else if (!tune && !paused && !IS_SD_PRINTING) {
    LCD_MESSAGEPGM(WELCOME_MSG);  
  }
...
//enum {ItemP_exit, ItemP_home, ItemP_disstep, ItemP_preheat_pla, ItemP_preheat_abs, ItemP_cooldown,/*ItemP_extrude,*/ItemP_move};
enum {ItemP_exit, ItemP_preheat_pla, ItemP_preheat_abs, ItemP_cooldown, ItemP_nozzle0, ItemP_bed};
...
*/
 ...
 updateActiveLines(ItemP_bed,encoderpos);
#endif
}

//////justin 
//enum {ItemP_exit, ItemP_preheat_pla, ItemP_preheat_abs, ItemP_cooldown};
...
enum {
  ItemAM_exit,
  ItemAM_X, ItemAM_Y, ItemAM_Z, ItemAM_E, ItemAM_R
};
...
void MainMenu::showAxisMove()
{
   ...
   for(int8_t i=lineoffset;i<lineoffset+LCD_HEIGHT;i++)
   {
     switch(i)
      {
          case ItemAM_exit:
          MENUITEM(  lcdprintPGM(MSG_CONTROL_ALT)  ,  BLOCK;status=Main_Control;beepshort(); ) ;
          break;
          case ItemAM_X:
          {
	 	  //oldencoderpos=0;
          ...
          MENUITEM(  lcdprintPGM(MSG_EXTRUDE)  ,  BLOCK;enquecommand("G92 E0");enquecommand("G1 F400 E5");beepshort(); ) ;
          break;
		  
          case ItemAM_R:
          // justin: added a retract 5 command
          MENUITEM(  lcdprintPGM(MSG_RETRACT)  ,  BLOCK;enquecommand("G92 E0");enquecommand("G1 F400 E-5");beepshort(); ) ;
          break;
          default:
          break;
      }
   ...
   updateActiveLines(ItemAM_R,encoderpos);
}
/*
enum {
  ItemAM2_exit,
  ItemAM_Z, ItemAM_E, ItemAM_R
};

void MainMenu::showAxisMove2()
{
   uint8_t line=0;
   int oldencoderpos=0;
   clearIfNecessary();
			...
			enquecommand("G91");
		      ...
		      enquecommand("G90");
                  ...
                  if(linechanging)
                  {
                   ...
                   { 
                        //justinjustin
                        old_z = ftostr52(current_position[Z_AXIS])
		    	enquecommand("G1 F500 Z0.5");
...
{ 
  ...
  case ItemT_speed:
    ...
    case ItemT_fan:
    {
      ...
      if(linechanging)
      {
        ...
        if(encoderpos>17) { //17*15=255
          encoderpos=17;
          //FanSpeed = 255;
        } 
        
        FanSpeed=encoderpos*15;
        analogWrite(FAN_PIN,  FanSpeed);
        lcd.setCursor(13,line);lcd.print(itostr3(FanSpeed));
      }
      ...
      if(force_lcd_update)
        ...
        case ItemT_moveZ:
        {
                if(force_lcd_update && paused)
                {
                  lcd.setCursor(0,line);lcdprintPGM(" Z:");
                  lcd.setCursor(12,line);lcd.print(ftostr52(current_position[Z_AXIS]));
                }
  			...
  			enquecommand("G91");
                ...
                if(linechanging)
                {
                 ...
                 { 
                      //justinjustin
                      char* command = "G92 Z";
                      char* command2 = ftostr52(current_position[Z_AXIS]);
                      //strcat(command,command2);
                      
                      char dest[12];
                      
                      strcpy( dest, command );
                      strcat( dest, command2 );
                      
                      //old_z = ftostr52(current_position[Z_AXIS]); 
                      enquecommand("G1 F500 Z0.05");
                      enquecommand(dest);
                      //lcd.setCursor(0,0);lcd.print(old_z);
  		      oldencoderpos=encoderpos;
                      encoderpos=0;
  		    }
 ...
 updateActiveLines(ItemT_moveZ,encoderpos);
}
...
//{ItemP_exit, ItemP_home, ItemP_disstep, ItemP_preheat_pla, ItemP_preheat_abs, ItemP_cooldown,/*ItemP_extrude,*/ItemP_move};
...
*/
enum {
  ItemC_exit, ItemC_home, ItemC_disstep, ItemC_move, ItemC_fan
};
...
void MainMenu::showControl()
{
 ...
 updateActiveLines(ItemC_fan,encoderpos);
}
  ...
  for(int8_t i=lineoffset;i<lineoffset+LCD_HEIGHT;i++)
  {
    ...
    { 
      case ItemM_watch:
       ...
       break;
       /*
      case ItemM_temperature:
        MENUITEM(  lcdprintPGM(MSG_TEMPERATURE); ,  BLOCK;status=Main_Temperature;beepshort(); ) ;
      break;
      */
      
      case ItemM_prepare:
        MENUITEM(  if(!tune && !paused) lcdprintPGM(MSG_PREPARE);else  lcdprintPGM(MSG_TUNE); ,  BLOCK;status=Main_Prepare;beepshort(); ) ;
      break;
       
      case ItemM_control:
        if (!tune && !paused){ MENUITEM(  lcdprintPGM(MSG_CONTROL_ARROW)  ,   BLOCK;status=Main_Control;beepshort(); ) ; }
        else if (!paused){ MENUITEM(  lcdprintPGM(MSG_PAUSE_PRINT)  ,   BLOCK;enquecommand("M25");paused=true; ) ; }
        else  MENUITEM(  lcdprintPGM(MSG_RESUME_PRINT)  ,   BLOCK;enquecommand("M24");paused=false; ) ;
      break;
      ...
      case ItemM_file:    
      {
        ...
        if((activeline==line)&&CLICKED)
        {
          ...
          if (card.sdprinting) 
          {
            ...
            enquecommand("G1 Z80 F2500"); //Justin
